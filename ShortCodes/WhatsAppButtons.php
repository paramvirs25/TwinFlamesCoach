<?php
function create_whatsapp_button($isAutoClick, $url, $text, $isConfirmClick){
    // Ensure correct class is added for auto-click
    $autoClickLinkText = ($isAutoClick) ? " auto-click-link" : ""; 
    $confirmClickText =  ($isConfirmClick) ? " confirm-click" : ""; 

    // Output WhatsApp button HTML
    $output = '<div class="wp-block-jetpack-send-a-message">';
    $output .= '<div class="wp-block-jetpack-whatsapp-button aligncenter is-color-dark' . $confirmClickText . '">';
...
function whatsapp_group_button_shortcode($atts, $content = null) {
    ...
    return create_whatsapp_button($isAutoClick, $atts['url'], $atts['text'], false);
}
